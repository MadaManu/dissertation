...
//         matrixa[i][j] = f48(count);
//         count++;
//     }
// }


// f48** matrixb = new f48*[4];
// for(int i = 0; i < 4; ++i){
//     matrixb[i] = new f48[4];
// }
...
// for(unsigned i=0;i<4;i++) {
//     for(unsigned j=0;j<4;j++) {
//         matrixb[i][j] = f48(count);
//         count++;
//     }
// }

// f48** matrixc = new f48*[4];
// for(int i = 0; i < 4; ++i){
//     matrixc[i] = new f48[4];
// }
...
do {
  ...
  switch(option) {
    ...
    case 3: cout<<"BLAS LEVEL 3 SELECTED!"<<endl;
        ...
        path = (char *)"./results/level3/reports/";
        sprintf(mkcmd, "mkdir -p %s/%d", path,size);
        res = system(mkcmd);
        if(res<0){
          cout<<"ERROR occured in creating folder structure.";
        ...
        do{
          ...
          cin >> inneroption;  // taking option value as input and saving in variable "option"
          switch(inneroption) {
            case 1: cout<<"Running BLAS l3 tests on doubles"<<endl;
              test_matrix_matrix_mul_double_nonSSE();
              cout<<endl;
              test_matrix_matrix_mul_double_SSE();
              cout<<endl;
              break;
            case 2: cout<<"Running BLAS l3 tests on f48"<<endl;
              test_matrix_matrix_mul_f48_nonSSE();
              cout<<endl;
              test_matrix_matrix_mul_f48_SSE();
              cout<<endl;
              break;
            case 3: cout<<"Building comparison report (double vs f48)"<<endl;
              cout<<"THIS NEEDS TO BE ADDEEEED!!!";
              break;
            case 4: cout<<"going back to main menu";
              break;
            default: cout<<"try again";
              break;
          }
